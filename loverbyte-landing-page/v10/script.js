...
(() => {
  ...
  function showDatingScene(name) {
    ...
    datingScenes.forEach((scene) => {
      const isActive = scene.getAttribute("data-dating-scene") === name;
  ...
  function runDating() {
    ...
    datingTimers.push(
      window.setTimeout(() => showDatingScene("toasts"), 2400),
    );

    // Moment 3: Match
    datingTimers.push(
      window.setTimeout(() => showDatingScene("match"), 6600),
    );

    // Moment 4: Private room + quiz receipt
    datingTimers.push(
      window.setTimeout(() => showDatingScene("private"), 10400),
    );
  }
