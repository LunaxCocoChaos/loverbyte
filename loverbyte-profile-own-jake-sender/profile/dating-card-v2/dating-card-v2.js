(() => {
  const mandatoryTopStripFields = [
    ...
    },
    {
      ...
      selectedValue: 'Prefer closer to my age/age range'
    }
  ];
  ...
  const settingsButton = document.querySelector('[data-dcv2-settings]');
  ...
  settingsButton?.addEventListener('click', () => {
    ...
    crushToast.textContent = 'Dating Card settings coming soon.';
    crushToast.hidden = false;
    if (toastTimer) window.clearTimeout(toastTimer);
    toastTimer = window.setTimeout(() => { crushToast.hidden = true; }, 1800);
  });
