(() => {
  ...
  const defaultProfileSnapshot = {
    displayName: 'Luna 👑',
    username: '@lunaiverseofficial',
    pronouns: 'self_describe',
    pronounsCustom: 'she/they/ai',
    ageBand: '30s',
    showAgeRange: false,
    bio: 'I drop prompts, clock patterns, and keep Loverbyte from turning into another dry dating app.\n\nI keep the love feed spicy, not dumpster-fire messy.\n\nSay hi. I don\'t byte 😘',
    occupation: 'Chief Tea Officer · Co-Host of Loverbyte',
    showOccupation: true,
    profilePhotoDataUrl: './Asset/luna-thinking.png',
    connectLinkLabel: 'Website',
    connectLinkUrl: 'https://loverbyte.app',
    cashAppLinkOrHandle: '$lunaonloverbyte',
    venmoLinkOrHandle: '',
    paypalLink: '',
  };
  ...
  const applyProfileSnapshot = () => {
    ...
    if (profileStats) {
      const followers = numberFormatter.format(Number.isFinite(Number(snapshot.followersCount)) ? Number(snapshot.followersCount) : 12987);
      const following = numberFormatter.format(Number.isFinite(Number(snapshot.followingCount)) ? Number(snapshot.followingCount) : 150);
