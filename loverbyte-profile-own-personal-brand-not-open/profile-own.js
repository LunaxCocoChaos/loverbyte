(() => {
  ...
  const profileRomanceSnapshot = {
    mode: 'not_open_to_crushes', // 'open_to_crushes' | 'not_open_to_crushes' | 'not_looking' | null
    seeking: 'F4M',
    ageBand: '30s',
    location: 'CA/USA',
    intent: 'Long-term',
  ...
  const defaultProfileSnapshot = {
    displayName: 'Isabel',
    username: '@isabelwithreceipts',
    pronouns: 'she_her',
    pronounsCustom: '',
    ageBand: '40s',
    showAgeRange: true,
    bio: 'From immigrant daughter to business exec. Proud carpenter\'s wife, hands-on mom to Juan. I read contracts, clock red flags, and believe love and property both need good foundations.',
    occupation: 'Corporate Lawyer · Board Member',
    showOccupation: true,
    profilePhotoDataUrl: '',
  ...
  const applyProfileSnapshot = () => {
    ...
    if (profileStats) {
      const followers = numberFormatter.format(Number.isFinite(Number(snapshot.followersCount)) ? Number(snapshot.followersCount) : 4398);
      const following = numberFormatter.format(Number.isFinite(Number(snapshot.followingCount)) ? Number(snapshot.followingCount) : 1567);
