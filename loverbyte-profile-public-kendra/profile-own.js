(() => {
  ...
  const defaultProfileSnapshot = {
    displayName: 'Kendra',
    username: '@kendratheblackgoddess',
    pronouns: '',
    pronounsCustom: '',
    ageBand: '20s',
    showAgeRange: false,
    bio: 'Bottle girl by night, accounting student by day. I like money 💰 organized, outfits loud, and romance with a man who knows how to plan cute 🥰 and sexy dates 😈.',
    occupation: 'Bottle girl @BreezyeBoo Nightclub',
    showOccupation: true,
    profilePhotoDataUrl: './assets/kENDRA.jpeg',
    connectLinkLabel: 'TikTok',
    connectLinkUrl: 'https://www.tiktok.com/',
    cashAppLinkOrHandle: '$kendraNYC',
    venmoLinkOrHandle: '',
    paypalLink: '',
  };
  ...
  const applyProfileSnapshot = () => {
    ...
    if (profileStats) {
      const followers = numberFormatter.format(Number.isFinite(Number(snapshot.followersCount)) ? Number(snapshot.followersCount) : 47);
      const following = numberFormatter.format(Number.isFinite(Number(snapshot.followingCount)) ? Number(snapshot.followingCount) : 23);
  ...
  /* ── Sender match modal ── */
  const senderMatchModal = document.createElement('div');
  senderMatchModal.className = 'lb-match-modal';
  senderMatchModal.setAttribute('role', 'dialog');
  senderMatchModal.setAttribute('aria-modal', 'true');
  senderMatchModal.setAttribute('aria-label', 'It\'s a match');
  senderMatchModal.hidden = true;
  senderMatchModal.innerHTML = `
    <div class="lb-match-modal__card">
      ...
      <div class="lb-match-modal__avatars">
        <div class="lb-match-modal__avatarCol">
          <img class="lb-match-modal__avatar" src="../loverbyte-profile-own-dating-card/jake_whitemaleheadshot.png" alt="Jake" decoding="async" />
          <span class="lb-match-modal__avatarName">Jake</span>
        </div>
        <div class="lb-match-modal__heart" aria-hidden="true"></div>
        <div class="lb-match-modal__avatarCol">
          <img class="lb-match-modal__avatar" src="../loverbyte-profile-own-dating-card/profile/dating-card-v2/Headshot.png" alt="Kelly" decoding="async" />
  ...
  `;
  document.body.appendChild(senderMatchModal);

  const openSenderMatchModal = () => {
    senderMatchModal.hidden = false;
    syncBodyScrollLock();
    requestAnimationFrame(() => {
      senderMatchModal.querySelector('[data-match-modal-close]')?.focus();
    });
  };

  const closeSenderMatchModal = () => {
    senderMatchModal.hidden = true;
    syncBodyScrollLock();
  };

  senderMatchModal.addEventListener('click', (e) => {
    if (e.target === senderMatchModal) closeSenderMatchModal();
  });

  senderMatchModal.querySelector('[data-match-modal-close]')?.addEventListener('click', closeSenderMatchModal);

  senderMatchModal.querySelector('[data-match-cta]')?.addEventListener('click', () => {
    showMiniToast('Clock My Tea flow coming soon.');
  });

  const addAcceptedCrushNotification = () => {
    if (!notifList) return;
    const item = document.createElement('li');
    item.className = 'lb-notifications__match';
    item.setAttribute('data-match-notif', '');
    item.innerHTML = `
      <p class="lb-notifications__matchCopy">${currentProfileHandle} accepted your Crush. The tea is officially hot 💘</p>
  ...
  let crushAlreadySent = false;

  const addCrushNotification = () => {
    if (crushAlreadySent) {
      showMiniToast(`Easy, romantic warrior. You already sent ${currentProfileHandle} a Crush 💘`);
      return;
    }
    crushAlreadySent = true;
    if (notifList) {
      const item = document.createElement('li');
      item.className = 'lb-notifications__crush';
      item.innerHTML = `You sent <strong>${currentProfileHandle}</strong> a Crush. Romantic courage logged 💘`;
      notifList.prepend(item);
    }
    if (notifBadge) {
    ...
    showMiniToast(`Crush sent to ${currentProfileHandle} 💘`);
    setTimeout(addAcceptedCrushNotification, 4000);
  };

  window.addEventListener('message', (e) => {
    if (e.data?.type === 'lb-crush-sent') addCrushNotification();
  });
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (event.key === 'Escape' && !senderMatchModal.hidden) {
      event.preventDefault();
      closeSenderMatchModal();
      return;
    }
